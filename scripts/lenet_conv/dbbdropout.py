...
net = lenet_conv(x, y, True, dropout=bbdropout, scale_kl=scale_kl,
        dep=True, center_init=center_init)
tnet = lenet_conv(x, y, False, reuse=True,
        dropout=bbdropout, scale_kl=scale_kl,
        dep=True, center_init=center_init)
...
def record():
    ...
    csvfn = os.path.join('../../records',
            'dbbdropout_lenet_conv.csv' if args.csvfn is None else args.csvfn)
