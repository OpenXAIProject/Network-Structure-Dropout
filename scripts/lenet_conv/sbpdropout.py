...
from model.lenet import lenet_conv
from model.sbpdropout import sbpdropout
...
scale_kl = 1e-2*N
dropout = sbpdropout
net = lenet_conv(x, y, True, dropout=dropout, scale_kl=scale_kl)
tnet = lenet_conv(x, y, False, reuse=True, dropout=dropout,
        scale_kl=scale_kl)
...
def record():
    ...
    csvfn = os.path.join('../../records',
            'sbpdropout_lenet_conv.csv' if args.csvfn is None else args.csvfn)
